...
public class Ex11_Array {
	...
	public static void main(String[] args) {
		...
		double[] doubleArray = {5.2, 2.3, 5.7, 4.5}; 	//초기값을 { }로 제공
		String[] strArray = new String[5] ;				// 개수만 정해서  배열생성
		
		int a;
		int[] b = new int[3];
//		System.out.println(a); 		//초기화하지 않은 변수a를 참조하므로 "에러" 
		System.out.println(b[0]); 	//배열은 기본값으로 초기화됨
		
		String s = "홍길동";
		System.out.println(s.length()); 	 //	3, s.length, method
		System.out.println(score.length);	 // length attribute

		System.out.println(score);
		System.out.println(Arrays.toString(score));  //[80, 70, 95, 68, 85], Arrays.
